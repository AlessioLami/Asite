...
const Conveyor5 = ({ hasError, hasWarning }: ErrorProps) => {
  ...
  const { scene } = useGLTF("/models/rullo5.glb");
      ...
      {status !== "ok" && (
        <>
          <Html position={[center.x, center.y, center.z]} center distanceFactor={30} occlude>
            ...
            >
              RULLO5 {status === "error" ? "ERRORE" : "WARNING"}
            </div>
          </Html>

          <group>
            <mesh ref={glowRef} rotation={[-Math.PI / 2, 0, 0]} position={[-13.5, 0, -13.7]}>
              <boxGeometry args={[14, 1.2, 0.1]} />
